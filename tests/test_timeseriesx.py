...
from timeseriesx import TimestampSeries, ureg
...
def default_timestamp_series(n=3):
    return TimestampSeries(
        pd.Series(
            PintArray(np.arange(n, dtype=float), dtype=ureg.parse_units("m")),
...
def test_timestamp_series_properties_start_default(default_timestamp_series):
    assert (
        default_timestamp_series.start
        == pd.Timestamp("2020-01-01T00:00:00").tz_localize("CET").to_pydatetime()
    )
...
def test_timestamp_series_properties_values_default(default_timestamp_series):
    assert default_timestamp_series.values == [0.0, 1.0, 2.0]
...
def test_timestamp_series_as_tuples_default(default_timestamp_series):
    assert default_timestamp_series.as_tuples() == [
        (pd.Timestamp("2020-01-01T00:00:00").tz_localize("CET").to_pydatetime(), 0.0),
        (pd.Timestamp("2020-01-02T00:00:00").tz_localize("CET").to_pydatetime(), 1.0),
        (pd.Timestamp("2020-01-03T00:00:00").tz_localize("CET").to_pydatetime(), 2.0),
    ]
...
def test_timestamp_series_as_pd_series_include_nan():
    ts = TimestampSeries(
        pd.Series(
            PintArray([0.0, np.nan, 2.0], dtype=ureg.parse_units("m")),
            index=pd.date_range("2020-01-01", freq="D", periods=3, tz="CET"),
        ),
        time_zone="CET",
        unit="m",
...
def test_timestamp_series_repr_default(default_timestamp_series):
    ...
    from pandas import DatetimeIndex, Series  # noqa: F401
...
def test_map_dimensionless(default_timestamp_series):
    mapped_ts = default_timestamp_series.map(lambda x: max(x, 1.0))
    assert mapped_ts.values == [1.0, 1.0, 2.0]
...
def test_map_with_dimension(default_timestamp_series):
    mapped_ts = default_timestamp_series.map(
        lambda x: max(x, 1.0 * ureg.Unit("m")),
        dimensionless=False,
    )
    assert mapped_ts.values == [1.0, 1.0, 2.0]
    assert mapped_ts._series.dtype.units == ureg.Unit("m")
...
def test_aggregate_without_returned_unit(default_timestamp_series):
    assert default_timestamp_series.aggregate(np.max) == 2.0


def test_aggregate_with_returned_unit(default_timestamp_series):
    assert default_timestamp_series.aggregate(
        np.max, with_unit=True
    ) == 2.0 * pint.Unit("m")
...
def test_sum_return_without_unit(default_timestamp_series):
    assert default_timestamp_series.sum() == 3.0


def test_sum_return_with_unit(default_timestamp_series):
    assert default_timestamp_series.sum(with_unit=True) == 3.0 * pint.Unit("m")
...
def test_mean_return_without_unit(default_timestamp_series):
    assert default_timestamp_series.mean() == 1.0


def test_mean_return_with_unit(default_timestamp_series):
    assert default_timestamp_series.mean(with_unit=True) == 1.0 * pint.Unit("m")
...
def test_round_with_unit():
    ...
    ts = TimestampSeries.create_from_lists(
        timestamps, values, time_zone="infer", unit="m"
    )
    ts.round(0)
    assert ts.values == [0.0, -2.0]
...
def test_timestamp_series_append_default(default_timestamp_series):
    assert default_timestamp_series.append(3.0).last == (
        pd.Timestamp("2020-01-04T00:00:00").tz_localize("CET").to_pydatetime(),
        3.0,
    )
...
def test_timestamp_series_append_empty():
    ts = TimestampSeries(
        pd.Series([], dtype=float, index=pd.DatetimeIndex([])), freq="10s"
    )
    with pytest.raises(ValueError):
        ts.append(1.0)
...
def test_timestamp_series_append_no_freq():
    ts = TimestampSeries(
        pd.Series([1.0], dtype=float, index=pd.DatetimeIndex(["2020-01-01"]))
    )
    with pytest.raises(ValueError):
        ts.append(1.0)
...
def test_timestamp_series_prepend_default(default_timestamp_series):
    assert default_timestamp_series.prepend(-1.0).first == (
        pd.Timestamp("2019-12-31T00:00:00").tz_localize("CET").to_pydatetime(),
        -1.0,
    )
...
def test_timestamp_series_prepend_empty(empty_timestamp_series):
    with pytest.raises(ValueError):
        empty_timestamp_series.prepend(1.0)
...
def test_create_timestamp_series_mismatching_time_zone_1():
    ...
    assert (
        ts.start
        == pd.Timestamp("2020-01-01T00:00:00").tz_localize("UTC").to_pydatetime()
    )
...
def test_create_timestamp_series_mismatching_compatible_unit():
    ts = TimestampSeries(
        series=pd.Series(
            PintArray([0.0, 1.0, 2.0], dtype="km"),
            index=[
    ...
    assert ts.values == [0.0, 1000.0, 2000.0]
...
def test_create_timestamp_series_valid_unit_str():
    ts = TimestampSeries(
        series=pd.Series([], index=pd.DatetimeIndex([]), dtype=np.float64),
        unit="second",
    )
...
def test_create_timestamp_series_valid_freq_str():
    ts = TimestampSeries(
        series=pd.Series([], index=pd.DatetimeIndex([]), dtype=np.float64), freq="30Min"
    )
...
def test_timestamp_series_add_timestamp_series_different_freq(default_timestamp_series):
    add_ts = TimestampSeries(
        pd.Series(
            PintArray(np.arange(3, dtype=float), dtype=ureg.parse_units("m")),
...
def test_timestamp_series_add_timestamp_series_different_tz(default_timestamp_series):
    ...
    assert (
        default_timestamp_series._series.index.union(add_ts._series.index).tolist()
        == result.timestamps
    )
    assert result.values == [0.0, 0.0, 1.0, 1.0, 2.0, 2.0]
...
def test_timestamp_series_add_timestamp_series_different_unit(default_timestamp_series):
    add_ts = TimestampSeries(
        pd.Series(
            PintArray(np.arange(3, dtype=float), dtype=ureg.parse_units("kg")),
            index=pd.date_range("2020-01-01", freq="D", periods=3, tz="CET"),
        ),
        time_zone="CET",
        unit="kg",
...
def test_timestamp_series_add_timestamp_series_different_index(
    default_timestamp_series,
):
    add_ts = TimestampSeries(
        pd.Series(
            PintArray(np.arange(3, dtype=float), dtype=ureg.parse_units("m")),
    ...
    assert result_ts.first == (pd.Timestamp("2020-01-01").tz_localize("CET"), 0.0)
    assert result_ts.values == [0.0, 1.0, 3.0, 2.0]
    assert result_ts.last == (pd.Timestamp("2020-01-04").tz_localize("CET"), 2.0)
...
def test_timestamp_series_add_pandas_series():
    ...
    pd_series = pd.Series(
        np.arange(3), index=pd.date_range("2020-01-02", freq="D", periods=3, tz="UTC")
    )
    result_ts = ts + pd_series
    assert result_ts.first == (pd.Timestamp("2020-01-01").tz_localize("UTC"), 0.0)
    assert result_ts.values == [0.0, 1.0, 3.0, 2.0]
    assert result_ts.last == (pd.Timestamp("2020-01-04").tz_localize("UTC"), 2.0)
...
def test_timestamp_series_add_pandas_series_different_time_zones():
    ...
    pd_series = pd.Series(
        np.arange(3.0), index=pd.date_range("2020-01-01", freq="D", periods=3, tz="UTC")
    )
    result_ts = ts + pd_series
    assert result_ts.first == (
        pd.Timestamp("2020-01-01").tz_localize("UTC").tz_convert("Europe/Stockholm"),
        0.0,
    )
    assert result_ts.values == [0.0, 2.0, 4.0]
    assert result_ts.last == (
        pd.Timestamp("2020-01-03").tz_localize("UTC").tz_convert("Europe/Stockholm"),
        4.0,
    )
    pd.testing.assert_series_equal(
        result_ts._series,
        pd.Series(
            [0.0, 2.0, 4.0],
            index=pd.date_range("2020-01-01", freq="D", periods=3, tz="UTC").tz_convert(
                "Europe/Stockholm"
            ),
        ),
    )
...
def test_timestamp_series_add_list_ok():
    ...
    result_ts = ts + [0.0, 1.0, 2.0]
    assert result_ts.values == [0.0, 2.0, 4.0]


def test_timestamp_series_add_pint_array(default_timestamp_series):
    result_ts = default_timestamp_series + PintArray([0.0, 100.0, 200.0], dtype="cm")
    assert result_ts.values == [0.0, 2.0, 4.0]
    assert result_ts.unit == ureg.Unit("m")


def test_timestamp_series_multiply_pint_array(default_timestamp_series):
    result_ts = default_timestamp_series * PintArray([1.0, 1.0, 1.0], dtype="m")
    assert result_ts.values == [0.0, 1.0, 2.0]
...
def test_timestamp_series_add_scalar():
    ...
    result_ts = ts + 1
    assert result_ts.values == [1.0, 2.0, 3.0]
...
def test_timestamp_series_div_scalar(default_timestamp_series):
    result_ts = default_timestamp_series / 2
    assert result_ts.values == [0.0, 0.5, 1.0]
...
def test_timestamp_series_subtract_list():
    ...
    result_ts = ts - [0.0, 1.0, 2.0]
    assert result_ts.values == [0.0, 0.0, 0.0]
...
def test_timestamp_series_subtract_pd_series_mismatch():
    ...
    result_ts = ts - pd.Series(
        np.arange(2), index=pd.date_range("2020-01-01", freq="D", periods=2)
    )
    assert result_ts.timestamps == ts.timestamps
    assert result_ts.values == [0.0, 0.0, 2.0]
...
def test_get_item_datetime_index(default_timestamp_series):
    assert default_timestamp_series[default_timestamp_series.start] == 0 * ureg.Unit(
        "m"
    )
...
def test_get_item_naive_datetime_index(default_timestamp_series):
    assert default_timestamp_series[
        default_timestamp_series.start.replace(tzinfo=None)
    ] == 0 * ureg.Unit("m")
...
def test_eq(default_timestamp_series):
    ts1 = TimestampSeries.create_from_tuples(
        [
            (pd.Timestamp(2020, 1, 1).tz_localize("UTC").tz_convert("EST"), 1.0),
            (pd.Timestamp(2020, 1, 2).tz_localize("UTC").tz_convert("EST"), 2.0),
        ],
        unit="km",
        freq=None,
    )
    ts2 = TimestampSeries.create_from_tuples(
        [
            (pd.Timestamp(2020, 1, 1).tz_localize("UTC").tz_convert("GMT"), 1000.0),
            (pd.Timestamp(2020, 1, 2).tz_localize("UTC").tz_convert("GMT"), 2000.0),
        ],
        unit="m",
        freq=None,
    )
...
def test_neq_1(default_timestamp_series):
    ts1 = TimestampSeries.create_from_tuples(
        [
            (pd.Timestamp(2020, 1, 1).tz_localize("UTC").tz_convert("EST"), 1.0),
            (pd.Timestamp(2020, 1, 2).tz_localize("EST"), 2.0),
        ],
        unit="km",
        freq=None,
    )
...
def test_neq_2(default_timestamp_series):
    ts1 = TimestampSeries.create_from_tuples(
        [
            (pd.Timestamp(2020, 1, 1).tz_localize("UTC").tz_convert("EST"), 1.0),
            (pd.Timestamp(2020, 1, 2).tz_localize("UTC").tz_convert("EST"), 3.0),
        ],
        unit="km",
        freq=None,
    )
...
def test_neq_3(default_timestamp_series):
    ts1 = TimestampSeries.create_from_tuples(
        [
            (pd.Timestamp(2020, 1, 1).tz_localize("UTC"), 1.0),
            (pd.Timestamp(2020, 1, 2).tz_localize("UTC"), 2.0),
        ],
        unit="m",
        freq=None,
    )
...
def test_timestamp_series_convert_unit_success(default_timestamp_series):
    ...
    assert np.array(ts.values).round(2).tolist() == [0.0, 0.54, 1.08]
    assert ts._series.dtype.units == ts.unit
...
def test_timestamp_series_fill_gaps():
    ...
    values = [1.0, 1.0, 1.0]
    ts = TimestampSeries.create_from_lists(
        timestamps, values, freq=pd.offsets.Hour(), time_zone="Europe/Berlin"
    )
    ts = ts.fill_gaps(
        ts.start - dt.timedelta(hours=1), ts.end + dt.timedelta(hours=1), value=2
    )
    assert len(ts) == 5
...
def test_timestamp_series_fill_gaps_start_and_end_different_timezone():
    ...
    values = [1.0, 1.0, 1.0]
    ts = TimestampSeries.create_from_lists(
        timestamps, values, freq=pd.offsets.Hour(), time_zone="UTC"
    )
    with pytest.raises(ValueError):
        ts.fill_gaps(
            pytz.timezone("Europe/Berlin").localize(timestamps[0])
            - dt.timedelta(hours=1),
            pytz.timezone("Europe/Berlin").localize(timestamps[-1])
            + dt.timedelta(hours=1),
            value=2,
        )
...
def test_timestamp_series_get_gaps():
    ...
    ts = TimestampSeries.create_from_lists(timestamps, values, freq=pd.offsets.Hour())
    gaps = ts.get_gaps(
        timestamps[0] - dt.timedelta(hours=1), timestamps[-1] + dt.timedelta(hours=1)
    )
    assert gaps == [
        timestamps[0] - dt.timedelta(hours=1),
        timestamps[-1] + dt.timedelta(hours=1),
...
def test_timestamp_series_get_gaps_no_start_and_end():
    ...
    values = [np.nan, 1.0, 1.0]
    ts = TimestampSeries.create_from_lists(timestamps, values, freq=pd.offsets.Hour())
    gaps = ts.get_gaps()
    assert gaps == [timestamps[0]]
...
def test_timestamp_series_resample_missing_values():
    ts = TimestampSeries(
        pd.Series(
            list(np.ones(12)) + list(np.repeat(np.nan, 12)),
